...
ivml.evaluate = function (attrib, d, i) {
  if (ivml.isF(attrib)){
    return attrib(d,i);
  }
  else{
...
ivml.getTextWidthInPixels = function (text, fontFamily, fontSize) {
  ...
  ivml.textCalculation.textCheckElement.style('font-family', fontFamily).style('font-size', fontSize).attr('class', 'ivml-text').text(text);
  return ivml.textCalculation.textCheckElement.node().getComputedTextLength();
};

ivml.getTextHeightInPixels = function (text, fontFamily, fontSize) {
  if (!ivml.textCalculation.textCheckElement) {
    ivml.textCalculation.textCheckElement = d3.select(document.body)
      .append('svg')
      .attr('height', 0)
  ...
  var theFontSize = fontSize !== null ? fontSize : d3.selectAll('.ivml-text').style('font-size');
  ivml.textCalculation.textCheckElement.style('font-family', fontFamily).style('font-size', theFontSize).classed('ivml-text', true).text(text);
  return ivml.textCalculation.textCheckElement.node().clientHeight;
};
...
ivml.generate_set_function_for_accessors = function (elementObject, accessorKey, redrawFunction) {
  return function (_x) {
    ...
    if (typeof(_x) == "string" && _x.length > 1 && (_x.charAt(0) === '/' || _x == 'null')) {
      if (_x == 'null') {
        elementObject.accessors[accessorKey] = null;
      }
      else {
...
ivml.generate_set_function_for_attributes = function (elementObject, attributeKey, redrawFunction) {
  return function (_x) {
    ...
    if (typeof(_x) == "string" && _x.length > 1 && (_x.charAt(0) === '/' || _x == 'null')) {
      if (_x == 'null') {
        elementObject.attributes[attributeKey] = null;
      }
      else {
        var key = _x.substring(1);
...
angular.module('ivml.chart', [])
  ...
  .directive('plot', function ($timeout, $window) {
    return {
      restrict: 'E',
      require: '^plot',
      controller: function () {
      ...
      link: function (scope, element, attr, controller) {
        ...
        // if window is a recognized object, use window.  Else, the calling controller can
        // broadcast resizeElement
        if (window.jQuery) {
          $(window).resize(resizePlot)
        } else {
          //scope.$on("resizeElement", resizePlot);
          angular.element($window).bind('resize', resizePlot);
        }
...
ivml.visualElements.Bars = function Bars(chartController, barController, $timeout) {
  ...
  for (var i in Bars.accessors) {
    Bars[i] = ivml.generate_set_function_for_accessors(Bars, i, function () {
      Bars.elements.barController.redraw()
    });
  }

  for (var i in Bars.attributes) {
    Bars[i] = ivml.generate_set_function_for_attributes(Bars, i, function () {
      Bars.elements.barController.redraw()
    });
  }

  for (var i in Bars.events) {
    Bars[i] = ivml.generate_set_function_for_events(Bars, i, function () {
      Bars.elements.barController.redraw()
    });
  }
...
ivml.visualElements.DonutCharts = function DonutCharts(controller, $timeout) {
  var DonutCharts = {
    ...
    accessors: {
      ...
      sliceFunction: null, //@ir function that returns a list of slices for the object
      sliceSizeFunction: null, //@ir function that determines the size of a slice
      sliceFillColorFunction: function (d, i) {      //@i function determining the fill color of a slice
  ...
  DonutCharts.redraw = function () {
    ...
    if (!DonutCharts.accessors.sliceSizeFunction) return;
    ...
    for (var d in data) {
      ...
      var pie = d3.layout.pie().value(DonutCharts.accessors.sliceSizeFunction).sort(null);
      ...
      var slices = DonutCharts.accessors.sliceFunction(data[d], d);

      var arcs = DonutCharts.elements.array_of_gs[on_index].selectAll('paths').data(pie(slices));

      arcs.enter().append("path")
        .attr("fill", function (d, i) {
          return d3.functor(DonutCharts.accessors.sliceFillColorFunction)(d.data, i);
        })
        .attr('stroke-opacity', function (d, i) {
          return d3.functor(DonutCharts.attributes.strokeOpacity)(d.data, i);
        })
        .attr('fill-opacity', function (d, i) {
          return d3.functor(DonutCharts.attributes.fillOpacity)(d.data, i);
        })
        .attr('stroke', function (d, i) {
          return d3.functor(DonutCharts.attributes.stroke)(d.data, i);
        })
        .attr("d", arc);
...
ivml.visualElements.Paths = function Paths(controller, lineGroupController, $timeout) {
  var Paths = {
    ...
    accessors: {
      ...
      points: [], //@ir returns an array of JavaScript objects that represent the points of the path.
  ...
  Paths.getAllPathsPoints = function () {
    if (Paths.elements.controller) {
      ...
      if (xs && ys && xf && yf && Paths.accessors.data) {
  ...
  Paths.redraw = function () {
    ...
    if (!Paths.accessors.points) return;
...
ivml.visualElements.Plot = function (element, controller, $timeout) {
  var Plot = {
    ...
    attributes: {
      ...
      plotLabelFontFamily: 'sans-serif',//@i font family for x axis
      ...
      resizeWidth: false, //@i set to true if dynamically resizes to parent div width during a resize event
      resizeHeight: false, //@i set to true if dynamically resizes to parent div height during a resize event
      ...
      xaxisFontSize: '12px',//@i font size for x axis
      ...
      yaxisFontSize: '12px',//@i font size for y axis
  ...
  Plot.redraw = function () {
    ...
    d3.select(Plot.elements.element).classed('ivplot', true);

    if (Plot.attributes.resizeWidth === true) {
      // ignore default height, width.  Calculate based on parent div
      Plot.attributes.width = getDynamicWidth(Plot.elements.element, 0, Plot.attributes.marginLeft, Plot.attributes.marginRight);

    }

    if (Plot.attributes.resizeHeight === true) {
      // ignore default height, width.  Calculate based on parent div
      Plot.attributes.height = getDynamicHeight(Plot.elements.element, 0, Plot.attributes.marginTop, Plot.attributes.marginBottom);
    }
    ...
    if (Plot.attributes.marginTop > 0) {
      //top
      coverAttrs.push({
        x: -Plot.attributes.marginLeft,
        y: -Plot.attributes.marginTop,
        width: Plot.attributes.width + Plot.attributes.marginLeft + Plot.attributes.marginRight,
        height: Plot.attributes.marginTop - lineThicknessAdjustment
      });
    }
    ...
    if (Plot.attributes.marginLeft > 0) {
      ...
      coverAttrs.push({
        x: -Plot.attributes.marginLeft,
        y: -Plot.attributes.marginTop,
        width: Plot.attributes.marginLeft - lineThicknessAdjustment,
        height: Plot.attributes.height + Plot.attributes.marginTop + Plot.attributes.marginBottom
      });
    }
    if (Plot.attributes.marginBottom > 0) {
      ...
      coverAttrs.push({
        x: -Plot.attributes.marginLeft,
        y: Plot.attributes.height + lineThicknessAdjustment,
        width: Plot.attributes.width + Plot.attributes.marginLeft + Plot.attributes.marginRight,
        height: Plot.attributes.marginBottom - lineThicknessAdjustment
      });
    }
    if (Plot.attributes.marginRight > 0) {
      //right
      coverAttrs.push({
        x: Plot.attributes.width + lineThicknessAdjustment,
        y: -Plot.attributes.marginTop,
        width: Plot.attributes.marginRight - lineThicknessAdjustment,
        height: Plot.attributes.height + Plot.attributes.marginTop + Plot.attributes.marginBottom
      });
    }
    ...
    Plot.elements.layers.xAxis.selectAll("text")
      .attr('class', 'ivml-text ivml-xaxis-text')
    ...
    Plot.elements.xAxisText
      .attr('class', 'ivml-text ivml-xaxis-label-text')
    ...
    Plot.elements.layers.yAxis.selectAll("text")
      .attr('class', 'ivml-text ivml-yaxis-text')
    ...
    var textHeight = ivml.getTextHeightInPixels(Plot.attributes.yaxisLabelText, Plot.attributes.yaxisFontFamily, Plot.attributes.yaxisFontSize);

    Plot.elements.yAxisText
      .attr('class', 'ivml-text ivml-yaxis-label-text')
      .attr('transform', 'translate(' + (textHeight - Plot.attributes.marginLeft) + ', ' + (-textHeight/2) + ')rotate(-90)')
    ...
    Plot.elements.plotLabelText
      .attr('class', 'ivml-text ivml-plot-label-text')
      .attr('transform', 'translate(0,' + (ivml.getTextHeightInPixels(Plot.attributes.plotLabelText, Plot.attributes.plotLabelFontFamily, Plot.attributes.plotLabelFontSize) - Plot.attributes.marginTop) + ')')
...
function getDynamicHeight(element, minHeight, marginTop, marginBottom) {
  //var height;
  //var parentHeightpx = d3.select(element.parentNode).style("height");
  //var parentHeight = parseInt(parentHeightpx, 10);
  var height = element.parentNode.offsetHeight - marginTop - marginBottom;
  if (height < minHeight) {
    return minHeight;
  }
  return height;
...
function getDynamicWidth(element, minWidth, marginLeft, marginRight) {
  //var width;
  //var parentWidthpx = d3.select(element.parentNode).style("width");
  //var parentWidth = parseInt(parentWidthpx, 10);
  var width = element.parentNode.offsetWidth - marginRight - marginLeft;
  if (width < minWidth) {
    return minWidth;
  }
  return width;
...
ivml.visualElements.Texts = function Texts(controller, $timeout) {
  var Texts = {
    ...
    attributes: {
      ...
      wrapWidth: null //@s wraps text to conform to width value.  Overrides maxWidth/truncateEnding attributes.
    },
    accessors: {
  ...
  Texts.redraw = function () {
    ...
    var textFunction = Texts.attributes.maxWidth || Texts.attributes.wrapWidth ?
      function (d, i) {
        if (Texts.attributes.wrapWidth) {
          return null;
        }
        ...
        if (maxWidth) {
          ...
          while (t.length && ivml.getTextWidthInPixels(displayString, ivml.evaluate(Texts.attributes.fontFamily, d, i), ivml.evaluate(Texts.attributes.fontSize, d, i)) > maxWidth) {
            t = t.slice(0, -1);
            displayString = t + end;
          }
        }
    ...
    var wrapText = function (d, i) {
      var maxWidth = ivml.evaluate(Texts.attributes.wrapWidth, d, i);
      var fontFamily = ivml.evaluate(Texts.attributes.fontFamily, d, i);
      var fontSize = ivml.evaluate(Texts.attributes.fontSize, d, i);
      var text = Texts.accessors.textFunction(d, i);
      var words = text ? text.split(/\s+/).reverse() : [];
      var word;
      var currentLine = '';
      var rtrn = [];
      while (word = words.pop()) {
        var checkLine = currentLine === '' ? word : currentLine + ' ' + word;

        var width = ivml.getTextWidthInPixels(checkLine, fontFamily, fontSize);
        if (width > maxWidth) {
          if (currentLine != '') {
            rtrn.push(currentLine);
            currentLine = '';
          }
        }
    ...
    var enterTexts = texts.enter().append('text');

    enterTexts
      .attr('class', 'ivml-text ivml-texts-element')
      .attr('x', xFunction)
      .attr('y', yFunction)
      ...
      .style('font-style', Texts.attributes.fontStyle)
    ...
    function appendWrapLines(d, i) {
      var selection = d3.select(this);
      selection.selectAll('tspan').remove();
      wrapText(d, i).forEach(function (d, i) {
    ...
    if (Texts.attributes.wrapWidth) {
      enterTexts.each(appendWrapLines);
    }
    else {
      enterTexts.text(textFunction)
    }

    var selection = ivml.animateSelection(texts
    ...
    if (Texts.attributes.wrapWidth) {
      selection.each(appendWrapLines);
    }
    else {
      enterTexts.text(textFunction)
    }
